...
| How many connections went to each destination subnet? | [count-connections-by-subnet.sh](count-connections-by-subnet.sh) | Chained `-C` calls, URL and CIDR parsing, counting with sort and uniq, reordered CSV columns |
...
You need Bash and `cho` on your `PATH`. The error and subnet summaries also use
`sort` and `uniq`. From the repository root, run:
