...
`s/part`で指定した位置が存在しない場合は、実行時エラーではなく空文字を返す
仕様でもよいかもしれない。特に、任意フィールドを抽出して後続の値式へ渡す用途では、
毎回`default`で回復するより空値のまま合成できるほうが自然な場合がある。一方で、
区切り文字が空、位置が不正、数値や日時などへの変換に失敗した場合まで空値にすると、
入力不備を見逃しやすくなるため、「構文・引数・型はエラー、存在しない対象だけ空値」
という境界を基本案とする。
...
- `s/part`の範囲外を空文字にするか、既存の実行時エラーを維持するか
